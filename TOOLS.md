...
- `create_workbook(filepath: str, sheet_name: str = "Sheet1") -> str`
  Creates a new workbook at `filepath` with the requested initial worksheet name. Fails if the destination file already exists.
...
- `apply_workbook_changeset(filepath: str, operations: List[Dict[str, Any]], assertions: Optional[List[Dict[str, Any]]] = None, mode: str = "preview", expected_workbook_sha256: Optional[str] = None, changeset_token: Optional[str] = None, create_snapshot: bool = True, snapshot_filepath: Optional[str] = None, sample_limit: int = 25) -> str`
  ...
  - `{"type": "freeze_panes_equals", "sheet_name": "Report", "cell": "A2"}`; use `null`, `""`, or `"A1"` to assert that panes are not frozen
  - `{"type": "autofilter_equals", "sheet_name": "Report", "range_ref": "A1:B3"}`
  - `{"type": "chart_exists", "sheet_name": "Report", "chart_type": "bar", "target_cell": "D2", "title": "Revenue by region", "width": 12, "height": 7}`; every field after `sheet_name` is optional, and provided dimensions are compared in centimeters with serialization tolerance
  ...
  Assertion cell/range references are normalized to canonical A1 form, so absolute references such as `$A$1:$B$3` compare correctly with workbook metadata. Value assertions distinguish booleans from numbers (`true` is not equal to `1`) while treating equivalent integer and floating-point numeric values as equal.
...
- `create_workbook_snapshot(filepath: str, snapshot_filepath: str) -> str`
  Creates a verified byte-for-byte `.xlsx` snapshot without overwriting an existing destination. The source is checked for concurrent changes while copying, both paths obey the active transport sandbox, and the response includes the snapshot path, size, and SHA-256 digest. Use this immediately before a risky mutation when you want a baseline for `diff_workbooks`.
...
- `describe_dataset(filepath: str, sheet_name: Optional[str] = None, table_name: Optional[str] = None, header_row: int = 1, sample_rows: int = 25, read_boundary_mode: str = "default") -> str`
  Returns a lightweight orientation summary for a worksheet or native Excel table, including `headers`, sample rows, inferred `schema`, header-quality signals, key-candidate guesses, structural observations, and a recommended follow-up read path. Worksheet-shaped summaries support `read_boundary_mode="strict"` (stop after any blank row), `"default"` (allow gaps of at most 5 blank rows), and `"extended"` (allow gaps of at most 100 blank rows). The `read_boundary` object reports the effective tolerance, data end, ignored trailing row count, and up to 10 compact trailing-block locations; `ignored_trailing_blocks_truncated=true` signals additional blocks. Non-default views are read-only diagnostics and return `write_precondition_compatible=false`; reread with the default mode before carrying a token into a structured write. Native Excel tables reject non-default modes because their boundaries are already explicit. If the selected sheet is a chart sheet, the response explains that and points at chart-oriented tools instead of failing.
...
- `read_excel_as_table(filepath: str, sheet_name: str, header_row: int = 1, start_row: Optional[int] = None, start_col: str = "A", end_col: Optional[str] = None, max_rows: Optional[int] = None, compact: bool = False, include_headers: bool = True, row_mode: str = "arrays", infer_schema: bool = False, read_boundary_mode: str = "default") -> str`
  Returns `headers`, `rows`, `total_rows`, `truncated`, `sheet_name`, and compact `read_boundary` metadata. Use `start_row` plus `max_rows` to paginate into deeper worksheet sections without reading from the top first, `start_col` / `end_col` to limit the width of wide sheets, and `include_headers=False` on follow-up pages after the first. Boundary presets are `strict` (0 blank rows), `default` (5), and `extended` (100); the bounded presets intentionally avoid a raw unlimited-gap knob. When more rows remain, the response includes `next_start_row` for the next page. Responses also include `structure_token`, `content_token`, and `snapshot_metadata`; only the default boundary view is marked as compatible with current structured-write preconditions.
- `quick_read(filepath: str, sheet_name: Optional[str] = None, header_row: int = 1, start_row: Optional[int] = None, start_col: str = "A", end_col: Optional[str] = None, max_rows: Optional[int] = None, include_headers: bool = True, row_mode: str = "arrays", infer_schema: bool = False, read_boundary_mode: str = "default") -> str`
  Returns a compact table from the requested sheet, or auto-selects the first workbook sheet when `sheet_name` is omitted. Use `start_row` for large-sheet pagination, `start_col` / `end_col` for narrower column slices, and `include_headers=False` for follow-up pages. Boundary presets are `strict` (0 blank rows), `default` (5), and `extended` (100), with the effective decision and any ignored trailing blocks returned under `read_boundary`. When more rows remain, the response includes `next_start_row` for the next page. Responses also include `structure_token`, `content_token`, and `snapshot_metadata`; reread with the default mode before using a token as a structured-write precondition.
- `row_mode`
